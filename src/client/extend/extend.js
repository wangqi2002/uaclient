...
export class GlobalExtensionManager {
    ...
    bindEventsToMain() {
        //绑定插件安装
        ipcClient.on(rendererEvents.extensionEvents.install, (event, workspace, extension) => {
            if (this.workspace.workspaceName != 'global' && workspace == 'global') {
                let gm = ClientStore.get(moduleStoreNames.extension, 'globalExtensionManager')
        ...
        //绑定插件卸载方法
        ipcClient.on(rendererEvents.extensionEvents.uninstall, (event, workspace, extension) => {
            if (this.workspace.workspaceName != 'global' && workspace == 'global') {
                let gm = ClientStore.get(moduleStoreNames.extension, 'globalExtensionManager')
                gm.enabledExtensions.push(extension)
                ClientStore.set(moduleStoreNames.extension, 'globalExtensionManager', gm)
        ...
        ipcClient.on(rendererEvents.workspaceEvents.create, (event, workspace, storage) => {
            if (workspace != 'global') {
                let m = {
                    attributes: {
